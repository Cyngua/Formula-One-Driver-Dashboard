function createParallelPlot() {
    ...
    d3.csv("../data/F1_driver_dataset.csv", rowConverter).then((data)=>  {
        ...
        window.parallelData = data;
        drawParallelPlot(svg, data, w, h, margin, 2020);            
    })
}


function drawParallelPlot(svg, data, w, h, margin, decade) {
...
function updateParallelPlot(decade) {
    const svg = d3.select("#plot2-canvas svg g");
    drawParallelPlot(svg, window.parallelData, 1000, 350, {top: 40, right: 20, bottom: 5, left: 80}, decade);
}